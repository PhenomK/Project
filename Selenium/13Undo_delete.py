...
class TaskClass(unittest.TestCase):
    ...
    def test1(self):
        ...
        elem5.send_keys("恢复删除测试文件夹"+ i )
        time.sleep(1)
        elem6 = self.driver.find_element_by_xpath("//span[@class='icon sure']")
        elem6.click()
        time.sleep(2)

        #勾选文件夹
        elem7 = self.driver.find_element_by_xpath("//a[@class='display-name' and @title='恢复删除测试文件夹" + i +"']/../../../..").find_element_by_class_name("item-checkbox") #勾选文件
        elem7.click()
        time.sleep(1)
        ...
        time.sleep(1)
        elem9 = self.driver.find_element_by_class_name("delete")                                         #点击删除
        elem9.click()
        elem10 = self.driver.find_element_by_xpath("//a[@class='dialog-button confirm-ok ok']")      #确认删除
        elem10.click()

        #恢复文件夹
        time.sleep(1)
        elem11 = self.driver.find_element_by_class_name("trash")                                         #选择已删除文件
        elem11.click()
        time.sleep(1)
        elem12 = self.driver.find_element_by_xpath("//a[@class='display-name' and @title='恢复删除测试文件夹" + i + "']/../../../../..").find_element_by_class_name("item-checkbox") #勾选删除文件
        elem12.click()
        time.sleep(1)
        elem13 = self.driver.find_element_by_id("undo")
        elem13.click()
        time.sleep(1)
        elem14 = self.driver.find_element_by_class_name("trash")                                         #选择已删除文件
        elem14.click()
        time.sleep(1)
